...
{ 20,	potionknown,	OPOTION,	0,	6	} ,
{ 90,	potionknown,	OPOTION,	1,	5	} ,
{ 520,	potionknown,	OPOTION,	2,	1	} , 	/* 40 */
{ 100,	potionknown,	OPOTION,	3,	2	} ,
{ 50,	potionknown,	OPOTION,	4,	2	} ,
{ 150,	potionknown,	OPOTION,	5,	2	} ,
{ 70,	potionknown,	OPOTION,	6,	1	} ,
{ 30,	potionknown,	OPOTION,	7,	7	} ,
{ 200,	potionknown,	OPOTION,	8,	1	} ,
{ 50,	potionknown,	OPOTION,	9,	1	} ,
{ 80,	potionknown,	OPOTION,	10,	1	} ,

/*cost	  memory 	iven name	iven arg   how
  gp	 pointer	  iven[]	ivenarg[]  many */

{ 30,	potionknown,	OPOTION,	11,	3	} ,
{ 20,	potionknown,	OPOTION,	12,	5	} ,	/* 50 */
{ 40,	potionknown,	OPOTION,	13,	3	} ,
{ 35,	potionknown,	OPOTION,	14,	2	} ,
{ 520,	potionknown,	OPOTION,	15,	1	} ,
{ 90,	potionknown,	OPOTION,	16,	2	} ,
{ 200,	potionknown,	OPOTION,	17,	2	} ,
{ 220,	potionknown,	OPOTION,	18,	4	} ,
{ 80,	potionknown,	OPOTION,	19,	6	} ,
{ 370,	potionknown,	OPOTION,	20,	3	} ,
{ 50,	potionknown,	OPOTION,	22,	1	} ,
{ 150,	potionknown,	OPOTION,	23,	3	} ,	/* 60 */
...
{ 100,	scrollknown,	OSCROLL,	0,	2	} ,
{ 125,	scrollknown,	OSCROLL,	1,	2	} ,
{ 60,	scrollknown,	OSCROLL,	2,	4	} ,
{ 10,	scrollknown,	OSCROLL,	3,	4	} ,
{ 100,	scrollknown,	OSCROLL,	4,	3	} ,
{ 200,	scrollknown,	OSCROLL,	5,	2	} ,
{ 110,	scrollknown,	OSCROLL,	6,	1	} ,	/* 70 */
{ 500,	scrollknown,	OSCROLL,	7,	2	} ,
{ 200,	scrollknown,	OSCROLL,	8,	2	} ,
{ 250,	scrollknown,	OSCROLL,	9,	4	} ,
{ 20,	scrollknown,	OSCROLL,	10,	5	} ,
{ 30,	scrollknown,	OSCROLL,	11,	3	} ,

/*cost	  memory 	iven name	iven arg   how
  gp	 pointer	  iven[]	ivenarg[]  many */

{ 340,	scrollknown,	OSCROLL,	12,	1	} ,
{ 340,	scrollknown,	OSCROLL,	13,	1	} ,
{ 300,	scrollknown,	OSCROLL,	14,	2	} ,
{ 400,	scrollknown,	OSCROLL,	15,	2	} ,
{ 500,	scrollknown,	OSCROLL,	16,	2	} , /* 80 */
{ 1000,	scrollknown,	OSCROLL,	17,	1	} ,
{ 500,	scrollknown,	OSCROLL,	18,	1	} ,
{ 340,	scrollknown,	OSCROLL,	19,	2	} ,
{ 220,	scrollknown,	OSCROLL,	20,	3	} ,
{ 3900,	scrollknown,	OSCROLL,	21,	0	} ,
{ 610,	scrollknown,	OSCROLL,	22,	1	} ,
{ 3000,	scrollknown,	OSCROLL,	23,	0	} ,
...
dnd_hed()
{
	...
	for (i=dnditm; i<IVENSIZE+dnditm; i++)
		dnditem(i);
	cursor(50,18);
	lprcat("You have ");
}
...
int i;
{
	...
	cursor( j+31,k );
	lprintf("%6d",((long)(itm[i].price*10L)));
}
...
lprcat("all inhabitants of the caves.  Here is the class schedule:\n\n\n");
	...
	/*line 7 of crt*/
	if (course[0]==0)
		lprcat("\t\ta)  Fighter's Training I        10 mobuls");
	lprc('\n');
	if (course[1]==0)
	...
	lprc('\n');
	if (course[7]==0)
		lprcat("\t\th)  History of Ularn             5 mobuls");
...
obanksub()
{
	...
	cursor(31,17);
	lprintf("You have %8d gold piece%s in the bank.",(long)c[BANKACCOUNT],
		c[BANKACCOUNT]==1?"":"s");
	cursor(40,18);
	lprintf("You have %8d gold piece%s",(long)c[GOLD],c[GOLD]==1?"":"s");
...
int eye, order;
{
	...
	lprintf("\nI see you have %s", objectname[OLARNEYE]);
	...
	if (getyn()=='y') {
		...
		lprintf("It is worth %d gold pieces to us\n",(long)amt);
		lprcat("Would you like to sell it? ");
		yrepcount=0;
		if (getyn()=='y') {
			lprcat("yes\n");
			c[GOLD]+=amt;
				...
				for (j=0; j<DNDSIZE; j++)
					if ((itm[j].obj == iven[isub]) ||
						...
						if ((iven[isub] == ODIAMOND)
						    ...
						    ||(iven[isub] == OEMERALD)
						    ||(iven[isub] == OSAPPHIRE))
						       value = (long)20*ivenarg[isub];
						else if((itm[j].obj == OSCROLL)
						    || (itm[j].obj == OPOTION))
							value = (long)2*itm[j+ivenarg[isub]].price;
						else {
							izarg=ivenarg[isub];
						/* appreciate if a +n object */
							value = (long)itm[j].price;
							if(izarg>=0) value *= 2;
							while ((izarg-- > 0) && ((value=14*(67+value)/10) < 500000L))
								;
						}
			lprintf("\nItem (%c) is worth %d gold piece%s to us.  Do you want to sell it? ",i,(long)value,value==1?"":"s");
						yrepcount=0;
						if (getyn()=='y') {
							lprcat("yes.\n");
							c[GOLD]+=value;
						...
						j = DNDSIZE+100;	/* get out of the inner loop */
					}
				if (j <= DNDSIZE+2)
...
pad_hd()
{
	...
	/*line 7 of crt*/
	if (drug[0]==0) lprcat("\t\ta)  Killer Speed\t\t100 bucks");
	lprc('\n');
	if (drug[1]==0) lprcat("\t\tb)  Groovy Acid\t\t\t250 bucks");
	lprc('\n');
	if (drug[2]==0) lprcat("\t\tc)  Monster Hash\t\t500 bucks");
	lprc('\n');
	if (drug[3]==0) lprcat("\t\td)  Trippy Shrooms\t\t1000 bucks");
	lprc('\n');
	if (drug[4]==0) lprcat("\t\te)  Cool Coke\t\t\t5000 bucks");
	lprc('\n');
...
opad()
{
	...
	while (1) {
		cursor(57,18);
		lprintf("%d buck%s on you.   ",(long)c[GOLD],
			c[GOLD]==1?"":"s");
		cursors();
		lprcat("\nSo, whaddya want [");
		standout("escape");
		...
		/*remember that he has snagged that drug*/
		if (flag)
			drug[i-'a']++;
		nap(1000);
	} /*end while(1) */
...
int foo;
{
	...
	case 'a': 	/* Ogre */
		...
		break;
	case 'b': 	/* Wizard */
		strcpy(char_class, "Wizard");
		spelknow[1]=1;	/* mle */
		spelknow[4]=1;	/* chm */
		c[SPELLMAX]=c[SPELLS]=2;
		c[HPMAX]=c[HP]=8;
		c[0] = 8;	/* strength */
		c[1] = 16;	/* intelligence */
		c[2] = 16;	/* wisdom */
		...
		c[5] = 8;	/* charisma */
		iven[0]=OPOTION;
		ivenarg[0]=19; /* potion of treasure detection */
		iven[1]=OSCROLL;
		ivenarg[1]=rund(6);
		iven[2]=OSCROLL;
		ivenarg[2]=rund(6);
	...
	case 'e':  	/* Rogue */
		...
		c[WIELD]=1;
		iven[2]=OSCROLL;
		ivenarg[2]=14;	/* stealth */
		break;
	case 'f':  	/* Geek */
		strcpy(char_class, "Geek");
		spelknow[0]=1;
		spelknow[1]=1;
		c[SPELLMAX]=c[SPELLS]=1;
		c[HPMAX]=c[HP]=10;
	...
	case 'g':  	/* Dwarf*/
		...
		break;
	case 'h':  	/* Rambo */
		strcpy(char_class, "Rambo");
		c[SPELLMAX]=c[SPELLS]=0;
		c[HPMAX]=c[HP]=1;
		c[0] = 3;	/* strength */
		c[1] = 3;	/* intelligence */
		c[2] = 3;	/* wisdom */
