...
void movemonst()
{
	...
	if (c[AGGRAVATE] || !c[STEALTH]) /* who gets moved? split for efficiency */
	{
		for (j=tmp1; j<tmp2; j++) /* look thru all locations in window */
			for (i=tmp3; i<tmp4; i++)
				if (mitem[i][j].mon)	/* if there is a monster to move */
...
void movemt(int i,int j)
{
	...
	switch(monst=mitem[i][j].mon)  /* for half speed monsters */
	{
	case TROGLODYTE:  
	case HOBGOBLIN:  
	case METAMORPH:  
	...
	tmp=0;
	for (x=0; x<IVENSIZE; x++)
		if (iven[x]==OHANDofFEAR){
			tmp=1;
			break;
		}
	if (c[SCAREMONST]) {
	...
	if (tmp) {
		...
		if ((tmp=item[xl][yl]) != OWALL)
			if (mitem[xl][yl].mon == 0)
				if ((mitem[i][j].mon != VAMPIRE) || (tmpitem != OMIRROR))
					if (tmp != OCLOSEDDOOR)		
						mmove(i,j,xl,yl);
		return;
	}
	...
	if (monster[monst].intelligence > 10-c[HARDGAME]) /* if smart monster */
	{
		...
		for (k=yl; k<yh; k++)
			for (m=xl; m<xh; m++) {
				if (level==1 && m==33 && k==(MAXY-1))
					screen[m][k] = 127;
				else if (monst>=DEMONPRINCE) 
					screen[m][k]=0;
				else
					switch(item[m][k]) {
					case OWALL: 
...
void mmove(int aa,int bb,int cc,int dd)
{
	...
	it=item[cc][dd];
		/* these monsters do not move onto pits or trapdoors */
	if ((it==OPIT) || (it==OTRAPDOOR))
		switch(mitem[aa][bb].mon) {
		case SPIRITNAGA:
		case PLATINUMDRAGON:
		case WRAITH:
		case VAMPIRE:
		...
		default:
			mitem[aa][bb].mon=0; /* fell in a pit or trapdoor */
			hitp[aa][bb]=0;
			show1cell(aa,bb);
			return;
		};
	for (i=0;i<mitem[aa][bb].n;i++) {
		mitem[cc][dd].it[i].item = mitem[aa][bb].it[i].item;
		mitem[cc][dd].it[i].itemarg = mitem[aa][bb].it[i].itemarg;
		mitem[aa][bb].it[i].item = 0;
		mitem[aa][bb].it[i].itemarg = 0;
	}
	mitem[cc][dd].n = mitem[aa][bb].n;
	mitem[aa][bb].n = 0;

	tmp = mitem[cc][dd].mon = mitem[aa][bb].mon;
	if (it==OANNIHILATION) {
		...
		if (tmp>=DEMONLORD) {
			...
			if (flag==0 
				...
				it=tmp=mitem[cc][dd].mon=0; /* monster dead */
		}
		else 
			it=tmp=mitem[cc][dd].mon=0; /* monster dead */
	}
	...
	if (tmp==LEMMING) {
		...
		*/
		if (rnd(100)<=2) {
			...
			mitem[aa][bb].mon = 0; /* no monster in old coords */
	}
	else 
		mitem[aa][bb].mon = 0; /* no monster in old coords */
	...
	if (tmp == LEPRECHAUN)/* leprechaun takes gold and gems */
		switch(it) {
		case OGOLDPILE:  
		case OMAXGOLD:  
		case OKGOLD:
	...
	if (tmp < DEMONLORD)
		if (it==OTELEPORTER)	/* monster hits teleport trap */
		{ 
			flag=3; 
			teleportmonst(cc, dd, mitem[cc][dd].mon);
		}

	if (tmp < DEMONLORD)
		if ((it==OELEVATORUP) || (it==OELEVATORDOWN)) {
			mitem[cc][dd].mon=0;
			flag=4;
		}
	...
	if (tmp >= DEMONLORD && tmp <= LUCIFER && c[EYEOFLARN] == 0)
		return;
	...
	for (trys=10; trys>0; --trys) {
		...
		if ((item[x][y]==0) && (mitem[x][y].mon==0) && 
			...
			for (i=0;i<mitem[xx][yy].n;i++) {
				mitem[x][y].it[i].item = 
					mitem[xx][yy].it[i].item;
				mitem[x][y].it[i].itemarg = 
					mitem[xx][yy].it[i].itemarg;
				mitem[xx][yy].it[i].item = 0;
